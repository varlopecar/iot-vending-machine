...
export const useNativePayment = () => {
  ...
  const [supportsApplePay, setSupportsApplePay] = useState<boolean | null>(
    null
  );
  const [supportsGooglePay, setSupportsGooglePay] = useState<boolean | null>(
    null
  );
  ...
  const checkNativePayAvailability = async () => {
    try {
      ...
      if (Platform.OS === "android") {
        try {
          // Note: isGooglePaySupportedAsync is not available in the current version
          // We'll need to implement this differently or check the Stripe documentation
  ...
  const initializeNativePayment = async (
    clientSecret: string,
    amount: number
  ) => {
    try {
  ...
  const presentNativePayment = async (): Promise<{
    success: boolean;
    error?: string;
  }> => {
    try {
