...
import { TabBarSpacer } from '../../components/ui';
...
export default function CartScreen() {
  ...
  return (
    <SafeAreaView className={`${isDark ? 'bg-dark-background' : 'bg-light-background'} flex-1`}>
      {/* Header */}
      <View className={`${isDark ? 'bg-dark-surface' : 'bg-light-surface'} p-4 border-b border-gray-300`}>
        <View className="flex-row items-center justify-between">
          ...
          <Text className={`${isDark ? 'text-dark-textSecondary' : 'text-light-text'} text-xl font-bold`}>
            Panier
          </Text>
          <View style={{ width: 20 }} />
        </View>
      ...
      {/* Footer */}
      <TabBarSpacer className={`${isDark ? 'bg-dark-surface' : 'bg-light-surface'} p-4 border-t border-gray-300`}>
        {/* Subtotal */}
