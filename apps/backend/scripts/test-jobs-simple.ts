...
/**
 * Script de test simple pour vérifier le bon fonctionnement des jobs
 * Usage: pnpm ts-node scripts/test-jobs-simple.ts
 */
...
async function testJobsSimple() {
  console.log('🚀 Démarrage du test simple des jobs...\n');
  ...
  try {
    ...
    console.log('  - Services accessibles via DI');
...
if (require.main === module) {
  testJobsSimple().catch((error) => {
    console.error('❌ Erreur fatale:', error);
    process.exit(1);
  });
}

export { testJobsSimple };
